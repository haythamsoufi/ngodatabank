...
class SessionService {
  ...
  Future<bool> isSessionValidForOffline() async {
    // Prefer the same local activity window as [isSessionExpired]: if the user
    // is still within the normal session lifetime, keep the logged-in shell,
    ...
    // Narrow grace for edge cases (e.g. stale metadata migration).
    final lastValidatedOnline = await getSessionLastValidatedOnline();
    if (lastValidatedOnline == null) return false;

    final timeSinceLastOnlineValidation =
        DateTime.now().difference(lastValidatedOnline);
