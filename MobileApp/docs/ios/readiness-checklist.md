...
- [ ] **GoogleService-Info.plist**: Add from Firebase Console → copy `GoogleService-Info.plist.example` to `ios/Runner/GoogleService-Info.plist` (real file is gitignored)
...
| Firebase Config | ⚠️ Local | Download plist from Firebase; path gitignored (see README) |
