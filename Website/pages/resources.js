...
export default function ResourcesPage() {
  ...
  if (!isLoaded) {
    return (
      <div className="w-full px-6 sm:px-8 lg:px-12 py-8">
        <Head>
          <title>Resources - Humanitarian Databank</title>
        </Head>
  ...
  if (error) {
    return (
      <div className="w-full px-6 sm:px-8 lg:px-12 py-8 text-center">
        <Head>
          <title>{`${t('resources.error.title')} - Humanitarian Databank`}</title>
        </Head>
        <h1 className="text-3xl font-bold text-humdb-red mb-6">{t('resources.error.title')}</h1>
        <p className="text-red-600 bg-red-100 p-4 rounded-md">{error}</p>
        <button onClick={() => router.reload()} className="mt-4 text-humdb-red hover:underline">
          {t('common.tryAgain')}
        </button>
      </div>
    );
  ...
  if (isLoading) {
    return (
      <div className="w-full px-6 sm:px-8 lg:px-12 py-8">
        <Head>
          <title>{`${t('resources.loading.title')} - Humanitarian Databank`}</title>
        </Head>
        <div className="text-center py-20">
          <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-humdb-red mb-4"></div>
          <h1 className="text-3xl font-bold text-humdb-navy mb-2">{t('resources.loading.title')}</h1>
          <p className="text-humdb-gray-600">{t('resources.loading.description')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <Head>
        <title>{`${t('resources.title')} - Humanitarian Databank`}</title>
        ...
        <div className="w-full h-full px-4 sm:px-6 lg:px-12 py-6 sm:py-8 lg:py-10">
          <div className="text-center mb-6 sm:mb-8">
           ...
           <form onSubmit={handleSearch} className="mb-6 sm:mb-8 max-w-2xl mx-auto px-4">
             ...
             <div className="relative border-2 border-humdb-gray-300 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-humdb-red focus-within:border-humdb-red">
               <input
                 id="resource-search"
                 type="text"
                 value={searchTerm}
                 ...
                 className="w-full px-4 sm:px-6 py-3 pr-20 sm:pr-24 text-humdb-gray-700 focus:outline-none border-none text-base"
               />
               <button
                 type="submit"
                 className="absolute right-0 top-0 h-full bg-humdb-red hover:bg-humdb-red-dark text-white font-semibold px-4 sm:px-6 transition-colors duration-150 text-base flex items-center justify-center"
               >
          ...
          <div className="mb-6 sm:mb-8">
                         <div className="flex items-center mb-4 sm:mb-6 px-4">
                               <h2 className="text-xl sm:text-2xl font-bold text-humdb-navy mr-3 sm:mr-4">
                  {t('resources.sections.publications.title')}
                  <span className="ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs sm:text-sm font-medium bg-humdb-red-light text-humdb-red border border-humdb-red/20">
                    {publicationsLoading ? '...' : publications.length}
                  </span>
                </h2>
               <div className="flex-1 h-px bg-humdb-gray-300"></div>
               <button
                 onClick={() => toggleSection('publications')}
                 className="ml-4 p-2 text-humdb-gray-600 hover:text-humdb-red transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-humdb-red focus:ring-offset-2 rounded-md"
              ...
              <>
                {publicationsLoading ? (
                  <div className="text-center py-6 sm:py-10 px-4">
                    <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-humdb-red mb-2"></div>
                    <p className="text-humdb-gray-600 text-sm sm:text-base">{t('common.loading')}</p>
                  </div>
                ) : publications.length === 0 ? (
                  <p className="text-center text-humdb-gray-600 text-base sm:text-lg py-6 sm:py-10 px-4">
                          ...
                          {publicationsCurrentPage > 1 && (
                            <li>
                              <button
                                ...
                                className="px-2 sm:px-3 py-2 text-xs sm:text-sm leading-tight text-humdb-gray-600 bg-white border border-humdb-gray-300 hover:bg-humdb-gray-100 hover:text-humdb-gray-700 transition-colors duration-150"
                              >
                                Previous
                              </button>
                            </li>
                          ...
                          {Array.from({ length: Math.max(1, publicationsTotalPages) }, (_, i) => i + 1).map((page) => (
                            <li key={page}>
                              <button
                                ...
                                className={`px-2 sm:px-3 py-2 text-xs sm:text-sm leading-tight border border-humdb-gray-300 transition-colors duration-150
                                  ${publicationsCurrentPage === page
                                    ? 'text-humdb-red bg-humdb-red-light border-humdb-red hover:bg-humdb-red-light font-semibold z-10'
                                    : 'text-humdb-gray-600 bg-white hover:bg-humdb-gray-100 hover:text-humdb-gray-700'
                                  }`}
                              >
                                {page}
                              </button>
          ...
          <div className="mb-6 sm:mb-8">
                         <div className="flex items-center mb-4 sm:mb-6 px-4">
                               <h2 className="text-xl sm:text-2xl font-bold text-humdb-navy mr-3 sm:mr-4">
                  {t('resources.sections.otherResources.title')}
                  <span className="ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs sm:text-sm font-medium bg-humdb-red-light text-humdb-red border border-humdb-red/20">
              ...
              <>
                {otherResourcesLoading ? (
                  <div className="text-center py-6 sm:py-10 px-4">
                    <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-humdb-red mb-2"></div>
                    <p className="text-humdb-gray-600 text-sm sm:text-base">{t('resources.sections.otherResources.loading')}</p>
                  </div>
                ) : otherResources.length === 0 ? (
                  <div className="bg-white rounded-lg border-2 border-dashed border-humdb-gray-300 p-4 sm:p-8 text-center mx-4">
                    <svg className="w-8 h-8 sm:w-12 sm:h-12 text-humdb-gray-400 mx-auto mb-3 sm:mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                    <h3 className="text-base sm:text-lg font-medium text-humdb-gray-600 mb-2">
                      {searchTerm ? t('resources.sections.otherResources.noResults') : t('resources.sections.otherResources.none')}
                    </h3>
                    <p className="text-humdb-gray-500 text-sm sm:text-base">
