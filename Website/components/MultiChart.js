            ...
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
                disaggregationView === 'sex'
                  ? 'bg-white text-ngodb-navy shadow-sm'
        ...
        {validData.map((item, index) => {
          ...
          return (
            <div key={index} className="flex items-start space-x-4 mb-6">
              <div className="w-48 text-sm text-ngodb-gray-600 leading-tight" title={item.label}>
              ...
              <div className="flex-1">
                {/* Stacked bar */}
                <div className="bg-ngodb-gray-200 rounded-full h-10 relative overflow-hidden mb-2">
                  {/* Stacked segments */}
                  {(() => {
    ...
    if (maxValue === minValue) {
      ...
      return (
        <div className="text-center py-8">
          <div className="bg-ngodb-gray-50 rounded-lg p-6 max-w-md mx-auto">
            <div className="text-2xl font-bold text-ngodb-red mb-2">
              {formatNumber(singleValue.value, showFullValues)}
            </div>
            <div className="text-lg text-ngodb-navy">
              {singleValue.label}
            </div>
          </div>
        </div>
