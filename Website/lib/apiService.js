...
/** Max distinct reporting years exposed in period dropdowns (newest first). */
const MAX_AVAILABLE_PERIODS_YEARS = 5;
...
function extractYearFromPeriod(periodName) {
  ...
  const yearMatch = String(periodName).match(/\b(20\d{2})\b/);
  if (yearMatch) {
    return parseInt(yearMatch[1], 10);
  }
  const numMatch = String(periodName).match(/\b(\d{4})\b/);
  if (numMatch) {
...
/**
 * Keep at most {@link MAX_AVAILABLE_PERIODS_YEARS} distinct calendar years (newest first).
 * One label per year (first seen when sorted newest-first).
 */
function limitPeriodsToLatestYears(periods, maxYears = MAX_AVAILABLE_PERIODS_YEARS) {
  if (!Array.isArray(periods) || periods.length === 0) return periods;
  const parsed = periods
    .filter((p) => p != null && String(p).trim() !== '')
    .map((p) => String(p).trim());
  const withYear = parsed.filter((p) => extractYearFromPeriod(p) > 0);
  if (withYear.length === 0) {
    return [...parsed].sort().reverse().slice(0, maxYears);
  }
  const sorted = [...withYear].sort((a, b) => extractYearFromPeriod(b) - extractYearFromPeriod(a));
  const seenYears = new Set();
  const out = [];
  for (const p of sorted) {
    const y = extractYearFromPeriod(p);
    if (seenYears.has(y)) continue;
    seenYears.add(y);
    out.push(p);
    if (out.length >= maxYears) break;
  }
  return out;
}
 ...
 * With local store enabled (default), derives periods from the synced data snapshot only — no Backoffice /periods wait.
...
export async function getAvailablePeriods(templateId = null) {
  ...
  const tid = templateId || FDRS_TEMPLATE_ID;

  const remember = (periods) => {
    const limited = limitPeriodsToLatestYears(periods);
    if (!Array.isArray(limited) || limited.length === 0) return limited;
    if (!getAvailablePeriods._cache) getAvailablePeriods._cache = {};
    getAvailablePeriods._cache[cacheKey] = { value: limited, ts: Date.now() };
    if (typeof window !== 'undefined') {
      try {
        window.localStorage.setItem(storageKey, JSON.stringify(getAvailablePeriods._cache[cacheKey]));
      } catch (_e) {}
    }
    return limited;
  };
  ...
  if (
    getAvailablePeriods._cache &&
    getAvailablePeriods._cache[cacheKey] &&
    Array.isArray(getAvailablePeriods._cache[cacheKey].value) &&
    Date.now() - getAvailablePeriods._cache[cacheKey].ts < 24 * 60 * 60 * 1000
  ) {
    return limitPeriodsToLatestYears(getAvailablePeriods._cache[cacheKey].value);
  }
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && Array.isArray(parsed.value) && Date.now() - parsed.ts < 24 * 60 * 60 * 1000) {
          if (!getAvailablePeriods._cache) getAvailablePeriods._cache = {};
          const limited = limitPeriodsToLatestYears(parsed.value);
          getAvailablePeriods._cache[cacheKey] = { value: limited, ts: parsed.ts };
          return limited;
        }
      }
    } catch (_e) {}
  ...
  // 3) Local snapshot only: derive distinct period_name from data.json (client uses /api/data?storeOnly — no Backoffice)
  if (USE_LOCAL_STORE && !FORCE_API) {
    try {
      const { getDataFromStore } = await import('./dataStore');
      const data = await getDataFromStore({ template_id: tid, storeOnly: true });
      if (Array.isArray(data) && data.length > 0) {
        const periods = [...new Set(data.map((item) => item.period_name).filter(Boolean))];
        if (periods.length > 0) {
          return remember(periods);
        }
      }
    } catch (error) {
      console.warn('Local store error for periods:', error?.message || error);
    }
      ...
      try {
        ...
        if (raw) {
          ...
          if (parsed && Array.isArray(parsed.value) && parsed.value.length > 0) {
            return remember(parsed.value);
          }
        }
      } catch (_e) {}
    }

    console.warn('Using default fallback periods (no period names in local data snapshot)');
    return limitPeriodsToLatestYears(['2024', '2023', '2022', '2021', '2020']);
  }

  // 4) Live Backoffice /api/v1/periods when local store is off or FORCE_API
  try {
    ...
    const periods = await fetchJsonWithCache(periodsUrl, {
      ttlMs: 24 * 60 * 60 * 1000,
      ...
      try {
        ...
        if (raw) {
          ...
          if (parsed && Array.isArray(parsed.value)) {
            return limitPeriodsToLatestYears(parsed.value);
          }
        }
      } catch (_e) {}
    }
...
/**
 * Fetch active embed content from the backoffice API.
 * @param {string} [category] - Optional category filter (e.g. 'global_initiative').
 * @returns {Promise<Array>} Array of embed objects.
 */
export async function getEmbedContent(category = '') {
  try {
    const params = {};
    if (category) params.category = category;
    const url = getBackofficeApiUrl('embed-content', params);
    const data = await fetchJsonWithCache(url, { ttlMs: 10 * 60 * 1000 });
    return data?.embeds || [];
  } catch (error) {
