...
const DATA_FILES = {
  ...
  embedContent: path.join(DATA_DIR, 'embed_content.json'),
...
async function applyFilters(data, filters) {
  filters = { ...filters };
  delete filters.storeOnly; // API hint only — not used when filtering rows
  ...
  if (!isServer) {
    ...
    try {
      ...
      Object.keys(filters).forEach(key => {
        if (key === 'storeOnly' || filters[key] === undefined || filters[key] === null) return;
...
export async function getEmbedContentFromStore(category = '') {
  if (!USE_LOCAL_STORE || FORCE_API) {
    throw new Error('Local store not enabled or API forced');
  }
  ...
  if (isServer) {
    try {
      const embeds = await loadDataFile('embedContent');
      if (!embeds || !Array.isArray(embeds)) return [];
      if (category) return embeds.filter(e => e.category === category);
      return embeds;
    } catch (error) {
      console.error('Error loading embed content from store:', error);
      return [];
    }
  }
  return [];
...
export async function syncDataFromBackend() {
  ...
  try {
    ...
    // 6. Fetch embed content
    let embedContent = [];
    try {
      const embedUrl = `${API_BASE_URL}/api/v1/embed-content?api_key=${API_KEY}`;
      const embedResponse = await fetch(embedUrl);
      if (embedResponse.ok) {
        const embedData = await embedResponse.json();
        embedContent = embedData.embeds || [];
        console.log(`   - ${embedContent.length} embed content items`);
      }
    } catch (error) {
      console.warn('⚠️ Failed to fetch embed content:', error.message);
    }

      // 7. Write to temporary files first (atomic writes)
      const tempFiles = {
        ...
        embedContent: DATA_FILES.embedContent + '.tmp',
    ...
    try {
      ...
      await fs.writeFile(tempFiles.embedContent, JSON.stringify(embedContent, null, 2));
      ...
      const metadata = {
        ...
        embed_content_count: embedContent.length || 0,
      ...
      await fs.rename(tempFiles.embedContent, DATA_FILES.embedContent);
