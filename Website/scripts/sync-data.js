...
async function syncData() {
  ...
  try {
    ...
    // Embed content (iframe/HTML snippets for public pages)
    let embedContent = [];
    try {
      const embedResponse = await fetchWithAuthFallback('/api/v1/embed-content');
      if (embedResponse.ok) {
    ...
    await fs.writeFile(
      path.join(DATA_DIR, 'embed_content.json.tmp'),
      JSON.stringify(embedContent, null, 2)
    );
    ...
    await fs.rename(
      path.join(DATA_DIR, 'embed_content.json.tmp'),
      path.join(DATA_DIR, 'embed_content.json')
    );
