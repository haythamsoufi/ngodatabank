...
@pytest.mark.usefixtures("db_session")
def test_cleanup_old_notifications_deletes_email_delivery_logs_first(app):
    """Scheduler cleanup must remove email_delivery_log rows before notification (FK)."""
    with app.app_context():
        email = f"cleanup_fk-{uuid.uuid4().hex}@example.com"
        user = User(
            email=email,
            name="Cleanup FK User",
            role="user",
            active=True,
        )
        user.set_password("test123")
        ...
        stats = cleanup_old_notifications(days=90)
