    ...
    def test_handle_assignment_form_get(self, client, db_session, app, admin_user):
        ...
        with app.app_context():
            ...
            with patch('app.services.authorization_service.AuthorizationService.can_access_assignment') as mock_access, \
                 ...
                 patch('app.routes.forms.TemplatePreparationService.prepare_template_for_rendering') as mock_prep:
    ...
    def test_save_submission_document_unique_storage_for_duplicate_filenames(self, db_session, app):
        ...
        with app.app_context():
            ...
            with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmpdir:
                ...
                rel1 = save_submission_document(f1, assignment_id=assignment_id, form_item_id=form_item_id, filename="report.pdf", is_public=False)
                rel2 = save_submission_document(f2, assignment_id=assignment_id, form_item_id=form_item_id, filename="report.pdf", is_public=False)

                assert rel1 != rel2
                assert rel1.startswith(f"assignments/{assignment_id}/{form_item_id}/")
                assert rel2.startswith(f"assignments/{assignment_id}/{form_item_id}/")

                abs1 = resolve_submission_file(rel1)
                abs2 = resolve_submission_file(rel2)
...
class TestFormProcessingUtilities:
    ...
    def test_get_form_items_for_section(self, db_session, app):
        ...
        with app.app_context():
            from app.utils.form_processing import get_form_items_for_section
...
class TestEntryFormPublicSubmissions:
    ...
    def test_view_public_submission(self, client, db_session, app, admin_user):
        ...
        with app.app_context():
            ...
            with patch("app.routes.forms.handle_public_submission_form", return_value="OK"):
