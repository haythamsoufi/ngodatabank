...
def _fdrs_reported_import_states() -> frozenset:
    """Env override: FDRS_REPORTED_IMPORT_STATES=100,200,300,400,500 (comma-separated IFRC State codes)."""
    raw = (os.environ.get("FDRS_REPORTED_IMPORT_STATES") or "").strip()
    if not raw:
        return DEFAULT_FDRS_REPORTED_IMPORT_STATES
    out = set()
    for part in raw.split(","):
        ...
        try:
            ...
            continue
    out = {x for x in out if x in ALLOWED_FDRS_ROW_STATES}
    return frozenset(out) if out else DEFAULT_FDRS_REPORTED_IMPORT_STATES


def resolve_reported_import_states(override: Optional[Sequence[int]] = None) -> FrozenSet[int]:
    """
    Resolve which IFRC reported-row State values keep a numeric value in the combined FDRS table.
    override=None: use FDRS_REPORTED_IMPORT_STATES env or default all except 0 (Not filled).
    """
    if override is not None:
        st = frozenset(int(x) for x in override)
        invalid = st - ALLOWED_FDRS_ROW_STATES
        if invalid:
            raise ValueError(
                "Invalid FDRS data status value(s): %s. Allowed IFRC State codes: %s."
                % (sorted(invalid), ", ".join(str(x) for x in sorted(ALLOWED_FDRS_ROW_STATES)))
            )
        if not st:
            raise ValueError("Select at least one data status to include.")
        return st
    return _fdrs_reported_import_states()


def _value_status_for_reported_state(state_i: int) -> str:
    if state_i == 500:
        return "Published Reported"
    w = _FDRS_STATE_WORD.get(state_i)
    if w:
        return f"Reported ({w})"
    return "Reported"
...
def _fdrs_reporting_year_key(year_raw: Any) -> Any:
    """
    Normalize API Year (int, float, date string like 01/01/2025, ISO) to int calendar year
    for grouping and assignment period joins (must match AssignedForm.period_name e.g. '2025').
    """
    if year_raw is None:
        return None
    if hasattr(year_raw, "year"):
        try:
            return int(year_raw.year)
        except Exception as e:
            logger.debug("_fdrs_reporting_year_key .year failed: %s", e)
    if isinstance(year_raw, bool):
        return int(year_raw)
    if isinstance(year_raw, int):
        return year_raw
    if isinstance(year_raw, float):
        if year_raw != year_raw or year_raw in (float("inf"), float("-inf")):
            return year_raw
        y = int(year_raw)
        return y if 1900 <= y <= 2100 else year_raw
...
def _fdrsdata_rows(
    ...
    value/Value, IntValue, BoolValue, StrValue, DateValue; year normalized to int calendar year when parseable.
    """
...
def fetch_fdrsdata(
    ...
    Expand DonCode, Year, KPICode->KPI_code, value/Value, IntValue, State, BoolValue, DateValue, StrValue;
...
def build_fdrs_combined(
    ...
    *,
    ...
    def _key_year(y_raw: Any) -> Any:
        yk = _fdrs_reporting_year_key(y_raw)
        return yk if isinstance(yk, int) else y_raw

    for r in reported_rows:
        y = _key_year(r.get("year"))
        key = (r.get("KPI_code") or "", r.get("DonCode") or "", y)
...
def build_fdrs_table(
    ...
    import_allowlist = resolve_reported_import_states(reported_import_states)
    api_min_status = min(import_allowlist)
    ...
    combined = build_fdrs_combined(
        reported, imputed, country_map, import_state_allowlist=import_allowlist
    )
