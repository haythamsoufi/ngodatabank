...
def main():
    try:
        allow_prod = str(os.environ.get('ALLOW_INIT_DATA_PROD', '')).strip().lower() == 'true'
        if os.environ.get('FLASK_CONFIG') == 'production' and not allow_prod:
            logger.warning("Refusing to run init_data in production without ALLOW_INIT_DATA_PROD=true (env: true/false only)")
            return 1
