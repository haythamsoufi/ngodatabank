...
class Config:
    # Application version (shown in System Configuration page header)
    APP_VERSION = "1.0.1"
    # GitHub repository for update checks (owner/repo)
    GITHUB_REPO = os.environ.get("GITHUB_REPO", "haythamsoufi/databank")
    ...
    if not _secret_key:
        if _flask_config == 'production':
            ...
            # Development/testing: Generate secure random key instead of predictable default
            import secrets
    ...
    #
    # SECURITY: Do NOT fall back to SECRET_KEY. SECRET_KEY secures Flask sessions/CSRF;
    ...
    # Validate API_KEY configuration
    #
    # NOTE:
    # The /api/v1 endpoints use database-managed API keys (api_keys table). The legacy
    # env/config API_KEY is kept only for backward compatibility and a small number of
    # shared-secret integrations (if configured to use it).
    if not API_KEY:
        _config_logger.info("API_KEY not set in environment.")
        _config_logger.info("   /api/v1 uses database-managed API keys; legacy API_KEY-based auth (if any) is disabled.")
