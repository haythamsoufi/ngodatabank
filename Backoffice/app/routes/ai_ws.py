    ...
    with app.app_context():
        try:
            ...
            if not convo:
                ...
                title = _build_initial_conversation_title(user_message)
                convo = AIConversation(
                    ...
                    meta={"client": client_meta, "title_source": "auto_initial"},
                )
            ...
            while True:
                ...
                if identity.is_authenticated and identity.user and conversation_id:
                    ...
                    if user_id_raw is not None:
                        try:
                            ...
                            convo = AIConversation.query.filter_by(id=conversation_id, user_id=user_id).first()
                            if not convo:
                                title = _build_initial_conversation_title(msg)
                                ...
                                convo = AIConversation(
                                    ...
                                    updated_at=utcnow(),
                                    last_message_at=utcnow(),
                                    meta={"client": client_meta, "source": "ws_init", "title_source": "auto_initial"},
                                )
                                db.session.add(convo)
                                db.session.commit()
                                _run_conversation_title_refine_background(
                ...
                try:
                    ...
                    def _ws_step(step_message: Optional[str] = None, detail: Optional[str] = None) -> None:
                        ...
                        try:
                            if step_message is None and detail is not None:
                                ...
                                if len(_d) > 500:
                                    _d = _d[:500] + "…"
                                current_app.logger.debug("AI WS sending step_detail: %s", _d)
                    ...
                    if _user_id is not None and conversation_id and (response_text or (getattr(result, "success", False) and has_visual)):
                        ...
                        client_meta = payload.get("client") or "unknown"
                        tid_persist = getattr(result, "trace_id", None)
                        _persist_ai_conversation_async(
                            ...
                            trace_id=int(tid_persist) if tid_persist is not None else None,
                        )
        ...
        try:
            while not cancelled.is_set():
                ...
                try:
                    ...
                    from app.routes.ai_documents.qa import (
                        _select_doc_scope,
                ...
                try:
                    from app.routes.ai_documents.helpers import _plan_query_with_llm, _resolve_country_from_text
                ...
                if use_full_document:
                    ...
                    try:
                        from app.routes.ai_documents.qa import _answer_with_full_documents
                ...
                try:
                    from app.routes.ai_documents.qa import _try_answer_from_table_records
                ...
                try:
                    from app.routes.ai_documents.qa import _try_answer_from_upr_metadata, _detect_participating_national_societies_intent
                ...
                if upr_hit:
                    ...
                    answer = upr_answer + " [1]."
                    try:
                        from app.routes.ai_documents.qa import _build_contextual_snippet
                    except Exception as e:
                ...
                try:
                    from app.routes.ai_documents.qa import _try_answer_participating_national_societies
                ...
                try:
                    from app.routes.ai_documents.qa import _try_answer_from_metric_blocks, _detect_metric_intent, _metric_query_text
