...
from app.utils.app_settings import get_organization_name, is_organization_email, user_has_ai_beta_access
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # SECURITY: Test user support only allowed in development environment
        # Test users must use environment-provided passwords, never hardcoded values
        flask_config = os.environ.get('FLASK_CONFIG', '').lower()
        is_production = flask_config == 'production'
        is_staging = flask_config == 'staging'
        is_development = flask_config == 'development' or flask_config == 'default'

        # Block test credentials in staging and production
        test_emails = ['test_admin@example.com', 'test_focal@example.com']
        if (is_production or is_staging) and submitted_email in test_emails:
            current_app.logger.warning(
                f"⚠️  SECURITY ALERT: Attempt to use test user ({submitted_email}) in {flask_config} environment. "
        ...
        # Block any attempt to use test credentials in production
        if is_production and submitted_email == 'sys-manager@example.com':
            current_app.logger.warning(
                f"⚠️  SECURITY ALERT: Attempt to use test user (sys-manager@example.com) in production environment. "
                "This is blocked for security reasons."
            )
            # Continue with normal authentication flow which will fail
        elif not is_production and submitted_email == 'sys-manager@example.com':
            # In development, allow test user creation/update but require environment-provided password
            test_password = os.environ.get('TEST_SYS_MANAGER_PASSWORD')
            if not test_password:
                # Generate secure random password for development
                import secrets
                test_password = secrets.token_urlsafe(16)
                current_app.logger.info(
                    f"⚠️  DEVELOPMENT: Generated secure password for sys-manager@example.com. "
                    f"Set TEST_SYS_MANAGER_PASSWORD environment variable to use a fixed password."
                )

            # Only proceed if provided password matches environment password
            if submitted_password == test_password:
                current_app.logger.warning(
                    f"⚠️  SECURITY WARNING: Test user login used for sys-manager@example.com in {flask_config} environment. "
                    "This feature is disabled in production."
                )
                try:
                    if not user:
                        user = User(email=submitted_email, name='System Manager')
                        user.set_password(test_password)
                        db.session.add(user)
                        db.session.flush()
                    else:
                        updated = False
                        # Update password to match environment password
                    ...
                    # Ensure RBAC system_manager role is assigned (best-effort)
                    try:
                        from app.models.rbac import RbacRole, RbacUserRole
                        sys_role = RbacRole.query.filter_by(code="system_manager").first()
                        if not sys_role:
                            sys_role = RbacRole(code="system_manager", name="System Manager", description="Full access (superuser).")
                            db.session.add(sys_role)
                            db.session.flush()
                        exists = RbacUserRole.query.filter_by(user_id=user.id, role_id=sys_role.id).first()
                        if not exists:
                            db.session.add(RbacUserRole(user_id=user.id, role_id=sys_role.id))
                            db.session.flush()
                    except Exception as e:
                        current_app.logger.warning(f"RBAC: could not assign system_manager role for dev user: {e}", exc_info=True)
                except Exception as e:
                    request_transaction_rollback()
                    current_app.logger.error(f"Error during test user setup: {e}", exc_info=True)
        ...
        if user and password_ok: # <-- Use check_password method
            # Generate session ID for tracking
            session_id = str(uuid.uuid4())
            session['session_id'] = session_id
            session['session_start'] = utcnow().isoformat()
            session['last_activity'] = utcnow().isoformat()  # Track last activity
...
def azure_login():
    ...
    # Store all pending auth states in a dict keyed by state value.
    # This prevents a second login attempt (new tab / double-click) from
    # overwriting the single b2c_state/verifier/nonce and causing a mismatch
    ...
    # Limit to 5 concurrent pending states to avoid cookie bloat
    if len(pending) > 5:
    ...
    # Use response_mode=query so Azure redirects with GET. With response_mode=form_post,
    # the callback is a cross-site POST and SameSite=Lax cookies are not sent, so
    # session (b2c_state, b2c_code_verifier) is lost and login always fails with state mismatch.
...
def azure_callback():
    ...
    # Ensure session is permanent for callback
    session.permanent = True

    # Look up this state in the pending-auth dictionary.
    # Falling back to the legacy single-key storage handles any in-flight logins
    # that started before this code was deployed.
    pending = session.get("b2c_pending") or {}
    auth_data = pending.pop(state, None)
    if auth_data is None:
        # Legacy fallback: single-key storage used before dict-based approach
        ...
        if state != stored_state:
            ...
            return redirect(url_for("auth.login"))
        # Found via legacy keys — extract and clear
    ...
    # nonce will be used for JWT verification below; make it accessible via session
    # only if not already set (the dict path provides it directly)
    ...
    claims = None
    if id_token:
        # Prefer secure verification for production readiness
        claims = _verify_and_decode_id_token(id_token, meta, cfg["client_id"], _nonce_for_verify)
        if not claims:
            # Fallback (shouldn't happen in normal conditions)
    ...
    # Establish session similar to password login
    session_id = str(uuid.uuid4())
    session['session_id'] = session_id
    session['session_start'] = utcnow().isoformat()
    session['last_activity'] = utcnow().isoformat()
    session.permanent = True
    # Store id_token so logout can pass it as id_token_hint to B2C end_session_endpoint
    if id_token:
...
def _generate_reset_token(email: str) -> str:
    ...
    Returns:
        The reset token string to be sent to the user
    """
    ...
    if user:
        try:
            ...
            # Continue anyway - token will still work, just won't be tracked
            request_transaction_rollback()
