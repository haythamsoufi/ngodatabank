...
def azure_login():
    ...
    session["b2c_code_verifier"] = verifier
    session["b2c_state"] = state
    session["b2c_nonce"] = nonce
    # Preserve 'next' for post-login redirect (e.g. /chat)
    next_url = request.args.get("next")
    if next_url and is_safe_redirect_url(next_url):
        session["b2c_next"] = next_url
...
def azure_callback():
    ...
    if state != stored_state:
        # Log for debugging
        current_app.logger.warning(
            f"State mismatch in Azure callback: received={state}, stored={stored_state}, "
            f"session_keys={list(session.keys())}"
        )
        flash(_("State mismatch. Please try signing in again."), "warning")
        return redirect(url_for("auth.login"))

    verifier = session.get("b2c_code_verifier")
    try:
    ...
    if id_token:
        ...
        claims = _verify_and_decode_id_token(id_token, meta, cfg["client_id"], session.get("b2c_nonce"))
        if not claims:
    ...
    # Clean PKCE vars and restore post-login redirect
    session.pop('b2c_code_verifier', None)
    session.pop('b2c_state', None)
    session.pop('b2c_nonce', None)
    next_page = session.pop('b2c_next', None)
