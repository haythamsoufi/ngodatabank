...
def _build_ai_groups():
    ...
    def _effective(key, default=None):
        ...
        if db_v is not None and (not isinstance(db_v, str) or db_v.strip()):
            return db_v
        # No DB row: reflect live Config (from env.py / config.py) so the UI matches
        # deployment, then fall back to the schema default from this form definition.
        try:
            cfg_v = getattr(Config, key, None)
        except Exception:
            cfg_v = None
        if cfg_v is not None and (not isinstance(cfg_v, str) or str(cfg_v).strip()):
            return cfg_v
        return default
    ...
    return [
        ...
        },
        {
            ...
            'fields': [
                ...
                {**txt('AI_QUERY_REWRITE_MODEL', 'Query Rewrite Model', 'Model for query rewrite. Blank uses default gpt-4o-mini (fast). Set only to override.'),
                 'show_when': [{'field': 'AI_QUERY_REWRITE_ENABLED', 'eq': True}]},
                {**txt('AI_TABLE_ENRICHMENT_MODEL', 'Table Enrichment Model', 'Recommended: gpt-4o-mini for speed on large country tables. Use the main chat model only if you need maximum quality (much slower). Clear the field to use the built-in default.', 'gpt-4o-mini')},
...
def _collect_ai_form_baseline_from_groups(ai_groups: list) -> dict:
    """Reproduce AI tab form field names/values as rendered (for partial-save merge)."""
    out: dict = {"ai_settings_present": "1"}
    for group in ai_groups or []:
        if group.get("id") == "providers" and group.get("provider_order") is not None:
            out["ai_provider_priority"] = json.dumps(group["provider_order"], ensure_ascii=False)
        field_blocks: list = []
        if group.get("provider_blocks"):
            for block in group.get("provider_blocks") or []:
                field_blocks.append(block.get("fields") or [])
            po = group.get("provider_other_fields")
            if po:
                field_blocks.append(po)
        else:
            field_blocks = [group.get("fields") or []]
        for fields in field_blocks:
            for f in fields:
                ft = f.get("type")
                if ft == "heading" or f.get("env_only"):
                    continue
                key = f.get("key")
                if not key:
                    continue
                ...
                if ft == "bool":
                    ...
                    if f.get("is_set") == "db" and f.get("value") is not None:
                        out[fk] = str(f["value"])
                elif ft == "select":
...
def _manage_settings_form_baseline(
    *,
    csrf_token: str,
    current_supported: list,
    current_show_language_flags: bool,
    current_doc_types: list,
    current_age_groups: list,
    current_sex_categories: list,
    doc_types_translations: dict,
    age_groups_translations: dict,
    sex_categories_translations: dict,
    current_entity_types: list,
    current_mobile_min_app_version: str,
    org_name_translations: dict,
    org_short_name_translations: dict,
    ...
    """Snapshot of POST body keys as the rendered settings form would submit (for partial-save merge)."""
    from app.models.enums import NotificationType

    base: dict = {
        "csrf_token": csrf_token or "",
        "languages": list(current_supported or []),
        "languages_order": ",".join(str(c).lower() for c in (current_supported or []) if c),
        "document_types[]": list(current_doc_types or []),
        "document_types_translations": json.dumps(doc_types_translations or {}, ensure_ascii=False),
        "age_groups[]": list(current_age_groups or []),
        "age_groups_translations": json.dumps(age_groups_translations or {}, ensure_ascii=False),
        "sex_categories[]": list(current_sex_categories or []),
        "sex_categories_translations": json.dumps(sex_categories_translations or {}, ensure_ascii=False),
        "enabled_entity_types[]": list(current_entity_types or []),
        "mobile_min_app_version": (current_mobile_min_app_version or "").strip(),
        "organization_name_translations": json.dumps(org_name_translations or {}, ensure_ascii=False),
        ...
        "chatbot_name": current_chatbot_name or "",
        "indicator_details_url_template": (current_branding or {}).get("indicator_details_url_template") or "",
        "propose_new_indicator_url": (current_branding or {}).get("propose_new_indicator_url") or "",
        "email_templates_present": "0",
    }

    if current_show_language_flags:
        base["show_language_flags"] = "1"
...
# Human labels for audit trail when saving settings (partial payload keys).
_MANAGE_SETTINGS_AUDIT_LABELS = {
    "mobile_min_app_version": "Minimum mobile app version",
    "languages": "Supported languages",
    "languages_order": "Language order",
    "show_language_flags": "Language flags in selectors",
    "document_types[]": "Document types",
    "document_types_translations": "Document type translations",
    "age_groups[]": "Age groups",
    "age_groups_translations": "Age group translations",
    "sex_categories[]": "Sex categories",
    "sex_categories_translations": "Sex category translations",
    "enabled_entity_types[]": "Enabled entity types",
    ...
    "organization_name_translations": "Organization name (translations)",
    "organization_short_name_translations": "Organization short name (translations)",
    "indicator_details_url_template": "Indicator details URL template",
    "propose_new_indicator_url": "Propose indicator URL",
    "email_templates_present": "Email templates",
    "ai_settings_present": "AI settings",
    "ai_provider_priority": "AI provider priority",
    "ai_beta_enabled": "AI beta access",
    "ai_beta_allowed_user_ids[]": "AI beta allowed users",
    "restart": "Restart app after save",
}


def _manage_settings_audit_label(key: str) -> str:
    if key in _MANAGE_SETTINGS_AUDIT_LABELS:
        return _MANAGE_SETTINGS_AUDIT_LABELS[key]
    ...
    if key.endswith("[]"):
        return key[:-2].replace("_", " ").strip().title()
    return key.replace("_", " ").strip().title()


def _format_manage_settings_audit_description(detail):
    """Build audit log description for manage_settings POST; None to use middleware default."""
    if not detail:
        return None
    keys = [k for k in (detail.get("keys") or []) if k and k not in ("csrf_token", "settings_partial_save")]
    if not keys:
        return None
    partial = bool(detail.get("partial"))
    labels = []
    for k in keys:
        labels.append(_manage_settings_audit_label(k))
    labels = [x for x in labels if x]
    if not labels:
        return None
    if not partial:
        return "Updated system settings (full configuration save)"
    if len(labels) <= 10:
    ...
    if request.method == "POST":
        ...
        if payload:
            try:
                ...
                partial_flag = str(decoded_obj.get("settings_partial_save") or "").lower() in (
                    "1",
                    "true",
                    "yes",
                )
                _audit_snap = dict(decoded_obj)
                _audit_snap.pop("settings_partial_save", None)
                _audit_snap.pop("csrf_token", None)
                settings_audit_detail = {"partial": partial_flag, "keys": list(_audit_snap.keys())}
                if partial_flag:
                    decoded_obj = dict(decoded_obj)
                    decoded_obj.pop("settings_partial_save", None)
                    token = decoded_obj.get("csrf_token") or data.get("csrf_token") or ""
                    baseline = _manage_settings_form_baseline(
                        csrf_token=token,
                    ...
                    merged = {**baseline, **decoded_obj}
                    data = _JsonFormProxy(merged)
                else:
            ...
            try:
                settings_audit_detail = {
                    "partial": False,
                    "keys": [k for k in request.form.keys() if k and k != "csrf_token"],
                }
            except Exception:
                settings_audit_detail = None
        try:
            ...
            if (
                ...
                with suppress(Exception):
                    adesc = _format_manage_settings_audit_description(settings_audit_detail)
                    if adesc:
                        g.audit_activity_description = adesc
...
@bp.route("/settings/branding-assets-upload", methods=["POST"])
@admin_permission_required("admin.settings.manage")
def branding_assets_upload():
    """Multipart upload for org logo/favicon before the JSON settings save. Azure Blob only."""
    from app.utils.branding_visual_assets import (
        branding_visual_upload_available,
        delete_branding_object_if_present,
        upload_organization_favicon,
        upload_organization_logo,
    )

    if not branding_visual_upload_available():
        return json_bad_request(
            "Branding file upload requires Azure Blob storage (AZURE_STORAGE_CONNECTION_STRING)."
        )

    paths = {}

    logo_f = request.files.get("organization_logo_file")
    if logo_f and logo_f.filename:
        try:
            current = (request.form.get("current_organization_logo_path") or "").strip()
            paths["organization_logo_path"] = upload_organization_logo(logo_f)
            delete_branding_object_if_present(current)
        except ValueError as e:
            return json_bad_request(str(e))
        except Exception:
            current_app.logger.exception("branding logo upload failed")
            return json_server_error("Could not upload logo.")

    fav_f = request.files.get("organization_favicon_file")
    if fav_f and fav_f.filename:
        try:
            current_f = (request.form.get("current_organization_favicon_path") or "").strip()
            paths["organization_favicon_path"] = upload_organization_favicon(fav_f)
            delete_branding_object_if_present(current_f)
        except ValueError as e:
            return json_bad_request(str(e))
        except Exception:
            current_app.logger.exception("branding favicon upload failed")
            return json_server_error("Could not upload favicon.")

    if not paths:
        return json_bad_request("No files uploaded.")

    return json_ok(paths=paths)
