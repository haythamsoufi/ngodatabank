...
def manage_assignments():
    assignments = AssignedForm.query.order_by(AssignedForm.period_name.desc()).all()
...
def assignments_gantt():
    ...
    assignments = AssignedForm.query.order_by(AssignedForm.assigned_at.asc()).all()

    # Group assignments by template
    template_groups = {}
    all_unique_countries = set()  # Track all unique countries across all assignments
    ...
    for assignment in assignments:
        ...
        if template_name not in template_groups:
            template_groups[template_name] = {
                'template_name': template_name,
                'assignments': [],
                'unique_countries': set(),  # Track unique countries for this template
                'active_count': 0
            }

        # Get the earliest due date from country statuses
        earliest_due_date = None
        if assignment.country_statuses.count() > 0:
            due_dates = [aes.due_date for aes in assignment.country_statuses if aes.due_date]
            if due_dates:
                earliest_due_date = min(due_dates)

        # If no due date set, use assigned_at + 30 days as default
        if not earliest_due_date:
            earliest_due_date = assignment.assigned_at + timedelta(days=30)

    # Get unique country IDs for this assignment
    assignment_country_ids = {aes.entity_id for aes in assignment.country_statuses}
    ...
    assignment_data = {
        ...
        'countries_count': len(assignment_country_ids),  # Count of unique countries
        ...
        try:
            ...
            if created_aes_list:
                ...
                if send_notifications:
                    try:
                        from app.utils.notifications import notify_assignment_created
