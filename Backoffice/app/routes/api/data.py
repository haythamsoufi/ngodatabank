...
def get_data_by_template(template_id):
    ...
    # Legacy behavior when no pagination requested - use centralized serialization
    all_form_data = []
    for data_item in assigned_form_data_query.all():
        all_form_data.append(serialize_form_data_item(data_item, 'assigned'))
    for data_item in public_form_data_query.all():
        all_form_data.append(serialize_form_data_item(data_item, 'public'))

    return json_response(all_form_data)
        ...
        if template_id is not None:
            try:
                ...
                if tmpl and getattr(tmpl, "published_version_id", None):
                    ...
                    # Fallback to earliest version when no published pointer exists (legacy templates)
                    fv = (
                        FormTemplateVersion.query
                        .filter(FormTemplateVersion.template_id == int(template_id))
                        .order_by(FormTemplateVersion.created_at.asc())
                        .first()
                    )
                    if fv and getattr(fv, "id", None):
                        published_version_id = int(fv.id)
            except Exception as e:
