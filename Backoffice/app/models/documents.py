...
class SubmittedDocument(db.Model):
    ...
    # country_id is only needed for standalone documents (not linked to assignments/submissions)
    # For form submission documents, country is derived from assignment_entity_status or public_submission
    country_id = db.Column(db.Integer, db.ForeignKey('country.id'), nullable=True)
    form_item_id = db.Column(db.Integer, db.ForeignKey('form_item.id'), nullable=True)
    filename = db.Column(db.String(255), nullable=False) # Original filename
    storage_path = db.Column(db.String(255), nullable=False) # Path to the stored file on the server
    uploaded_at = db.Column(db.DateTime, default=utcnow, nullable=False)
    uploaded_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False) # User who uploaded the file
    document_type = db.Column(db.String(255), nullable=True) # Document type/category
    language = db.Column(db.String(10), nullable=True) # Language code (e.g., 'en', 'fr', 'es', 'ar')
    is_public = db.Column(db.Boolean, default=False, nullable=False) # Whether document is publicly accessible
    period = db.Column(db.String(100), nullable=True) # Period the document covers (e.g., '2024', '2024-2025', 'Jan-Dec 2024')
    status = db.Column(db.String(50), nullable=False, default='Pending')  # 'Pending', 'approved', 'rejected'

    # Thumbnail fields
    thumbnail_filename = db.Column(db.String(255), nullable=True)  # Original filename of the thumbnail
    thumbnail_relative_path = db.Column(db.String(512), nullable=True)  # Relative path to thumbnail file
...
class Resource(db.Model):
    ...
    def get_translation(self, language_code):
        """Get translation for specific language, fallback to default if not found"""
        translation = self.translations.filter_by(language_code=language_code).first()
        return translation
