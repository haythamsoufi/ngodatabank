...
class AIJob(db.Model):
    ...
    job_type = Column(String(64), nullable=False, index=True)  # e.g. "docs.bulk_reprocess"
    ...
    meta = Column(JSONB, nullable=True)  # job-level settings (concurrency, etc.)
...
class AIJobItem(db.Model):
    ...
    item_index = Column(Integer, nullable=False)  # 0-based stable order within a job

    # Optional linkage to a target entity for convenience (also present in payload).
    entity_type = Column(String(64), nullable=True, index=True)  # e.g. "ai_document"
    ...
    payload = Column(JSONB, nullable=True)  # workflow-specific input for this item
