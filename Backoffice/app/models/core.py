...
class User(UserMixin, db.Model):
    ...
    def set_password(self, password):
        # Ensure a cross-environment compatible default
        # Explicitly use PBKDF2-SHA256 so new hashes work on Fly and locally
...
class UserLoginLog(db.Model):
    ...
    email_attempted = db.Column(db.String(120), nullable=False)  # Store email even if user doesn't exist

    # Event details
    event_type = db.Column(db.String(20), nullable=False)  # 'login_success', 'login_failed', 'logout'
...
class UserActivityLog(db.Model):
    ...
    # Activity details
    activity_type = db.Column(db.String(50), nullable=False)  # 'page_view', 'form_submit', 'file_upload', etc.
