...
def get_client_ip():
    ...
    # Check for forwarded IP first (common in production behind load balancers)
...
def log_login_attempt(email, success=True, user=None, session_id=None, failure_reason=None):
    ...
    try:
        ...
        is_suspicious = check_suspicious_login(client_info['ip_address'], email, success)
        ...
        if not success or is_suspicious:
            create_security_event_for_login(email, success, is_suspicious, client_info, failure_reason)
...
def check_suspicious_login(ip_address, email, success):
    ...
    recent_emails = db.session.query(func.count(func.distinct(UserLoginLog.email_attempted))).filter(
        and_(
            UserLoginLog.ip_address == ip_address,
            UserLoginLog.timestamp > utcnow() - timedelta(hours=1)
        )
    ...
    # Check for successful login from new location for this user
    if success:
        user = User.query.filter_by(email=email).first()
        if user:
            previous_ips = db.session.query(func.distinct(UserLoginLog.ip_address)).filter(
                and_(
            ...
            if ip_address not in previous_ip_list and len(previous_ip_list) > 0:
                return True
...
def create_security_event_for_login(email, success, is_suspicious, client_info, failure_reason):
    ...
    if is_suspicious:
        create_security_event(
            ...
            severity='medium',
            description=f"Suspicious login attempt for {email} from {client_info['ip_address']} (Reason: {failure_reason or 'N/A'})",
            context_data={
                'email': email,
                'success': success,
                'failure_reason': failure_reason,
