...
def translate_notification_message(translation_key: str, params: Optional[Dict[str, Any]] = None, locale: Optional[str] = None) -> str:
    ...
    # Debug logging only (keep INFO logs clean)
    try:
        from app.utils.form_localization import get_translation_key
        current_locale = get_translation_key()
        current_app.logger.debug(
            f"[NOTIFICATION_TRANSLATE] Key: {translation_key}, "
            f"Requested locale: {locale}, current locale: {current_locale}"
        )
    except Exception as e:
        # Non-critical; do not fail translation if locale introspection fails
        current_app.logger.debug(f"[NOTIFICATION_TRANSLATE] Error getting locale info: {e}")

    # Notification translation source strings (English)
    # These are translated at runtime using the current request locale
    translation_sources = {
        # Assignment notifications
        'notification.assignment_created.title': 'New Assignment Created',
        'notification.assignment_created.message': "A new assignment '%(template)s' for period '%(period)s' has been created. Due date: %(due_date)s",

        'notification.assignment_submitted.title': 'Team Update: %(template)s submitted',
        'notification.assignment_submitted.message': "Another focal point in your entity submitted '%(template)s' for period %(period)s. No action needed from you.",

        'notification.assignment_submitted.admin.title': 'Please review assignment - {template} for {country}',
        'notification.assignment_submitted.admin.message': "{submitter_name} has submitted this assignment for period {period}. Please review and validate this submission in the Backoffice.",

        'notification.assignment_approved.title': 'Assignment Approved',
        'notification.assignment_approved.message': "Assignment '%(template)s' has been approved",

        'notification.assignment_reopened.title': 'Assignment Reopened',
        'notification.assignment_reopened.message': "Assignment '%(template)s' has been reopened",

        # Document notifications
        'notification.document_uploaded.title': 'Document Uploaded',
        'notification.document_uploaded.message': "Document '%(document)s' (%(document_type)s) was uploaded",

        'notification.document_uploaded.pending.title': 'Document Uploaded - Pending Review',
        'notification.document_uploaded.pending.message': "Document '%(document)s' (%(document_type)s) has been uploaded and requires approval",

        # Access request notifications
        'notification.access_request_received.title': 'Country Access Request Received',
        'notification.access_request_received.message': '%(user_name)s requested access to %(country_name)s',

        # User notifications
        'notification.user_added_to_country.title': 'User Added to Country',
        'notification.user_added_to_country.message': 'You have been added as a focal point for %(country)s',

        # Template notifications
        'notification.template_updated.title': 'Template Updated',
        'notification.template_updated.message': "Template '%(template)s' has been updated",

        # Form notifications
        'notification.form_updated.title': 'Form Updated',
        'notification.form_updated.message': "Form data has been updated",

        # Deadline notifications
        'notification.deadline_reminder.title': 'Deadline Reminder',
        'notification.deadline_reminder.message': "Assignment '%(template)s' is due on %(due_date)s",

        # Self-report notifications
        'notification.self_report_created.title': 'Self Report Created',
        'notification.self_report_created.message': "A new self-report has been created",

        # Public submission notifications
        'notification.public_submission_received.title': 'Public Submission Received',
        'notification.public_submission_received.message': "A new public submission for '%(template)s' from %(submitter)s (%(country)s) has been received",

        # Admin message notifications (these use custom messages, so we handle them specially)
        'notification.admin_message.title': 'Admin Message',  # Default, can be overridden with custom_title in params
        'notification.admin_message.message': 'You have received an admin message',  # Default, can be overridden with message in params
    }
    ...
    if translation_key in translation_sources:
        try:
            ...
            if locale_to_use:
                current_app.logger.debug(
                    f"[NOTIFICATION_TRANSLATE] Using locale: {locale_to_use} for key: {translation_key}, source: '{source_string[:60]}...'"
                )
                try:
                    ...
                    try:
                        ...
                        # Refresh failures should not break notifications, but should be visible in logs.
                        current_app.logger.debug(
                            "[NOTIFICATION_TRANSLATE] refresh() failed (continuing): %s",
                            e_refresh,
                            exc_info=True,
                        )
                    ...
                    with force_locale(locale_to_use):
                        ...
                        if str(translated) == source_string and locale_to_use != 'en':
                            ...
                            current_app.logger.debug(
                                f"[NOTIFICATION_TRANSLATE] Translation FOUND for locale '{locale_to_use}'. "
                                f"'{source_string[:50]}...' -> '{str(translated)[:50]}...'"
                            )
                except Exception as e:
                ...
                current_app.logger.debug(
                    f"[NOTIFICATION_TRANSLATE] Using current locale: {current_babel_locale} (no explicit locale passed)"
                )
                ...
                if str(translated) == source_string and current_babel_locale != 'en' and current_babel_locale != 'unknown':
                    ...
                    current_app.logger.debug(
                        f"[NOTIFICATION_TRANSLATE] Translated '{source_string[:50]}...' -> '{str(translated)[:50]}...' (locale: {current_babel_locale})"
                    )
            ...
            if params and isinstance(params, dict):
                ...
                if format_params:
                    try:
                        ...
                        current_app.logger.debug(f"[NOTIFICATION_TRANSLATE] Formatted result: '{result[:80]}...'")
                        return result
...
def create_notification(
    ...
    try:
        ...
        from app.utils.app_settings import get_notification_priority
...
def notify_assignment_submitted(assignment_entity_status):
    ...
    admin_notifications = create_notification(
        ...
        title_params={'template': template_name, 'country': entity_name},
...
def notify_standalone_document_uploaded(document, country_id):
    ...
    try:
        ...
        # Log activity (standalone documents are country-specific for now)
        log_entity_activity(
            entity_type='country',
            entity_id=country_id,
