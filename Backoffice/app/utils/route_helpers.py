...
def normalize_value_for_display(value, *, preserve_matrix_keys=True):
    """Simplify verbose labels like ``'Total: 123'`` to ``'123'`` for summaries.

    When *preserve_matrix_keys* is ``True`` (default), labels that contain
    digits or underscores (e.g. matrix-style keys like ``"123_EFs"``) are
    kept intact so they are not mistakenly stripped.

    Args:
        value: The value to normalize.
        preserve_matrix_keys: When True, only strip generic text labels.
        ...
        if isinstance(value, str):
            ...
            if len(parts) == 2:
                ...
                if candidate_digits.isdigit():
                    if preserve_matrix_keys and any(ch.isdigit() or ch == '_' for ch in label):
                        return value
                    return candidate
        return value
