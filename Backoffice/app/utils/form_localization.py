        ...
        if isinstance(translations_dict, dict):
            ...
            if translated_name and translated_name.strip():
                ...
                current_app.logger.debug(
                    f"[TEMPLATE_LOCALIZATION] Found translation for template {template.id} "
                    f"(name='{name_to_use}'): locale='{current_locale}', translated='{translated_name[:50]}...'"
                )
            ...
            if translated_name and translated_name.strip():
                from flask import current_app
                current_app.logger.debug(
                    f"[TEMPLATE_LOCALIZATION] Found translation (lowercase) for template {template.id}: "
                    f"locale='{current_locale.lower()}', translated='{translated_name[:50]}...'"
                )
                return translated_name

            # Log available keys for debugging
            from flask import current_app
            available_keys = list(translations_dict.keys())
            current_app.logger.debug(
                f"[TEMPLATE_LOCALIZATION] No translation found for template {template.id} "
                f"(name='{name_to_use}'): requested locale='{current_locale}', available keys={available_keys}"
            )
