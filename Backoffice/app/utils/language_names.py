...
@lru_cache(maxsize=1)
def _all_languages_display_names_map() -> dict:
    """Static ISO 639-1 → English name map from app config (Babel omits many rare codes)."""
    try:
        from config.config import Config

        return dict(getattr(Config, "ALL_LANGUAGES_DISPLAY_NAMES", {}) or {})
    except Exception:
        return {}


def _english_name_from_config(base: str) -> Optional[str]:
    name = _all_languages_display_names_map().get(base)
...
def language_endonym(code: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        # CLDR has no data for many valid ISO 639-1 codes; not an operational issue.
        return None
...
def language_display_name(code: Optional[str], viewer_locale: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        unknown_to_babel = True
    except Exception as e:
        logger.debug("language_display_name failed for %s: %s", code, e)

    cfg_name = _english_name_from_config(base)
    if cfg_name:
        return _capitalize_first_cased_char(cfg_name)

    if not unknown_to_babel:
        endo = language_endonym(code)
        if endo:
            return endo
    return None
