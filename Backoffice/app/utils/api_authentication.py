...
def authenticate_db_api_key_only():
    ...
    # Backward compatibility: allow a legacy static API key configured in app settings.
    # This is used by internal scripts that call /api/v1 endpoints via HTTP.
    legacy_key = str(current_app.config.get("API_KEY") or "").strip()
    if legacy_key and provided_key == legacy_key:
        # Return a lightweight non-Response object to indicate success
        return {"legacy_api_key": True}
...
def authenticate_api_request():
    ...
    # Backward compatibility: allow legacy static API key configured in app settings.
    legacy_key = str(current_app.config.get("API_KEY") or "").strip()
    if provided_key and legacy_key and provided_key == legacy_key:
        elevated_access = True
        api_key_record = None
        g.api_key_record = None
        return (elevated_access, auth_user, api_key_record)

    # First, try to authenticate with database-managed API keys
    if provided_key:
        try:
