...
- If only document tools are available, call search_documents early and answer from excerpts (best-effort). If evidence is insufficient, suggest enabling other document sources.
...
- **Documents only** ("only from documents", "from reports", "from plans", "in the PDFs"): use ONLY search_documents (or search_documents_hybrid). Do NOT call databank tools.
