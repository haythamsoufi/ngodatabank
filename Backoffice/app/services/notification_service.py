...
from app.utils.notifications import get_default_icon_for_notification_type, validate_and_sanitize_action_buttons
import logging
...
class NotificationService:
    ...
    def _translate_notification_content(cls, notification: Notification) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            from app.utils.notifications import translate_notification_message
            from app.utils.form_localization import get_translation_key
            ...
            if title_key:
                try:
                    logger.debug(
                        f"[NOTIFICATION_SERVICE] Translating title_key '{title_key}' for notification {notification.id}, "
                        f"locale: {locale_to_use}"
                    )
                    # Pass locale explicitly
                    translated_title = translate_notification_message(title_key, title_params, locale=locale_to_use)
                    logger.debug(
                        f"[NOTIFICATION_SERVICE] Translated title for notification {notification.id}: "
                        f"'{translated_title[:80] if translated_title else 'None'}...'"
                    )
                except Exception as e:
            ...
            if message_key:
                try:
                    ...
                    if message_key in keys_requiring_country and 'country' not in message_params:
                        ...
                        logger.debug(
                            f"[NOTIFICATION_SERVICE] Checking for missing 'country' parameter for notification {notification.id}: "
                            f"entity_type='{entity_type}', entity_id={entity_id}, message_params keys={list(message_params.keys())}"
                        )
                        if entity_type and entity_id:
                            try:
                                ...
                                logger.debug(
                                    f"[NOTIFICATION_SERVICE] Entity lookup result for notification {notification.id}: "
                                    f"entity_name='{entity_name}'"
                                )
                                if entity_name and not entity_name.startswith('Unknown'):
                                    message_params['country'] = entity_name
                                    logger.debug(
                                        f"[NOTIFICATION_SERVICE] Added missing 'country' parameter for notification {notification.id}: "
                                        f"entity_type='{entity_type}', entity_id={entity_id}, country='{entity_name}'"
                                    )
                                else:
                                    logger.debug(
                                        f"[NOTIFICATION_SERVICE] Entity name lookup returned invalid result for notification {notification.id}: "
                                        f"entity_name='{entity_name}'"
                                    )
                            except Exception as e:
                    ...
                    logger.debug(
                        f"[NOTIFICATION_SERVICE] Checking template localization for notification {notification.id}: "
                        f"message_params={message_params}, has_template={'template' in message_params if message_params else False}"
                    )
                    if message_params and 'template' in message_params:
                        try:
                                ...
                                try:
                                    ...
                                    if assigned_form:
                                        logger.debug(
                                            ...
                                            f"Found AssignedForm {assigned_form.id} directly (related_object_id={notification.related_object_id})"
                                        )
                                    else:
                                        ...
                                        if assignment_status:
                                            if assignment_status.assigned_form:
                                                ...
                                                logger.debug(
                                                    f"[NOTIFICATION_SERVICE] Notification {notification.id}: "
                                                    f"Found AssignedForm {assigned_form.id} via AssignmentEntityStatus {notification.related_object_id}"
                                                )
                                            else:
                                                logger.warning(
                                                    f"[NOTIFICATION_SERVICE] Notification {notification.id}: "
                                    ...
                                    if assigned_form:
                                        ...
                                        # Fallback: try template relationship (shouldn't normally be needed)
                                            ...
                                            try:
                                                ...
                                                logger.debug("db.session.expire/refresh failed: %s", e)
                                            logger.debug(
                                                f"[NOTIFICATION_SERVICE] Notification {notification.id}: "
                                                f"Using relationship template (fallback) for AssignedForm {assigned_form.id}"
                                            )
                                        else:
                                            template = None
                                            logger.warning(
                            ...
                            if template:
                                ...
                                logger.debug(
                                    f"[NOTIFICATION_SERVICE] Notification {notification.id} details: "
                                    f"notification_type={notification.notification_type}, "
                                    f"entity_type={notification.entity_type}, "
                                    f"entity_id={notification.entity_id}, "
                                    f"message_params={notification.message_params}"
                                )
                        except Exception as e:
                    ...
                    logger.debug(
                        f"[NOTIFICATION_SERVICE] Translating message_key '{message_key}' for notification {notification.id}, locale: {locale_to_use}"
                    )
                    translated_message = translate_notification_message(message_key, message_params, locale=locale_to_use)
                    logger.debug(
                        f"[NOTIFICATION_SERVICE] Translated message for notification {notification.id}: "
                        f"'{translated_message[:80] if translated_message else 'None'}...'"
                    )
                except Exception as e:
    ...
    def get_user_notifications(
        ...
        try:
            ...
            # Batch load all AssignmentEntityStatus records
            assignment_status_cache = {}  # {aes_id: AssignmentEntityStatus}
            assigned_form_cache = {}  # {form_id: AssignedForm}
            if assignment_status_ids or assigned_form_ids:
                try:
                    from app.models.assignments import AssignmentEntityStatus, AssignedForm

                    # Batch load all AssignmentEntityStatus records that might be referenced
                    ...
                    # Batch load all AssignedForm records that might be referenced
                    if assigned_form_ids:
                        ...
                        # Also batch load all AssignmentEntityStatus records for these forms
                        if form_records:
                            ...
                            # Index by (assigned_form_id, entity_type, entity_id) for quick lookup
                            # Also store by ID for direct lookups if not already stored
                            for aes in aes_for_forms:
                                ...
                                # Also store by ID if not already in cache
            ...
            for n in notifications:
                ...
                if not entity_name and n.related_object_type == 'assignment' and n.related_object_id:
                    try:
                        ...
                        logger.debug(
                            f"[NOTIFICATION_SERVICE] Entity fallback lookup failed for notification "
                            f"{n.id} (related_object_id={n.related_object_id}): {entity_err}"
                        )
