...
def get_organization_logo_path(default: str = "logo.svg") -> str:
    ...
    Missing or blank values use ``default`` (Movement emblem ``logo.svg``, same artwork as
    ``Website/public/ns_logo_placeholder.svg``).
    """
    branding = get_organization_branding()
    raw = branding.get("organization_logo_path")
    if raw is None:
        return default
    path = str(raw).strip()
...
def get_organization_favicon_path(default: str = "favicon.svg") -> str:
    ...
    Missing or blank values use ``default`` (bundled ``favicon.svg``, same emblem as ``logo.svg``).
    """
    branding = get_organization_branding()
    raw = branding.get("organization_favicon_path")
    if raw is None:
        return default
    path = str(raw).strip()
    return path or default
...
def _strip_visual_path_prefixes(relative_path: str) -> str:
    """Strip leading slashes and repeated ``static/`` so ``url_for('static', …)`` does not double-prefix."""
    n = (relative_path or "").replace("\\", "/").strip().lstrip("/")
    while n.lower().startswith("static/"):
        n = n[7:].lstrip("/")
    return n


def organization_visual_asset_href(path: Optional[str], default: str = "logo.svg") -> str:
    """Resolve a logo/favicon *path* to an href with asset cache-busting (like ``static_url``).

    Values may be bundled static filenames (e.g. ``logo.svg``) or uploaded keys
    (``branding/<uuid>_logo.png``) served via the uploads API when using Azure Blob storage.
    """
