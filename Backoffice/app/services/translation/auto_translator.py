...
class AutoTranslator:
    ...
    def _protect_variables(text: str) -> Tuple[str, Dict[str, str]]:
        ...
        # Bounded quantifiers prevent polynomial backtracking on adversarial input
        named_fmt = re.compile(r"%\([^)]{1,100}\)[#0\- +]{0,8}\d{0,10}(?:\.\d{1,10})?[sdfoxX]")
        # Positional: %s, %d, %.2f, etc. (avoid %% and avoid named formats)
        positional_fmt = re.compile(r"%(?!%)(?!\()[#0\- +]{0,8}\d{0,10}(?:\.\d{1,10})?[sdfoxX]")
