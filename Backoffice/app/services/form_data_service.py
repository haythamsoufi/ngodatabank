...
from app.utils.debug_utils import (
    debug_manager, performance_monitor, debug_form_data, debug_request_info,
    debug_database_query, log_user_action, format_error_context
)
from app.utils.form_processing import (
    FormItemProcessor,
...
from app.utils.notifications import notify_document_uploaded
from app.utils.file_paths import save_submission_document, resolve_submission_file
...
class FormDataService:
    ...
    def _is_verbose_logging_enabled() -> bool:
        ...
        This controls detailed per-item logging that can be excessive in production.
        Set VERBOSE_FORM_DATA_LOGGING=true in environment to enable.
        """
    ...
    def process_form_submission(cls, assignment_entity_status, all_sections: List, csrf_form=None) -> Dict[str, Any]:
        ...
        """
        # Enhanced debugging with new utilities
        ...
        try:
            ...
            for section in all_sections:
                ...
                if section.section_type == 'repeat':
                    ...
                    logger.debug("Repeat processing completed for section %s with %s changes", section.id, len(repeat_changes))
    ...
    @classmethod
    @performance_monitor("Section Data Processing")
        ...
        for file_data in valid_files:
            ...
            try:
                ...
                try:
                    ...
                    notify_document_uploaded(assignment_entity_status, secured_filename)
                except Exception as e:
            ...
            if form_key.startswith('delete_document_') and request.form[form_key] == 'true':
                try:
                    ...
                    try:
                        ...
                        try:
                            abs_path = DocumentService._resolve_storage_path(doc_to_delete.storage_path)
                            if os.path.exists(abs_path):
                                os.remove(abs_path)
                        except Exception as e:
                            # Deleting DB row should still proceed, but leaving orphaned files should be observable
                            logger.warning(
                                f"Failed to delete document file from disk (will still delete DB row): "
        ...
        if edit_doc_id_key in request.form and request.form[edit_doc_id_key]:
            ...
            try:
                ...
                if edit_file_key in request.files and request.files[edit_file_key].filename:
                    ...
                    # Remove old file (resolve path first)
                    from app.services.document_service import DocumentService
                    try:
                        old_abs_path = DocumentService._resolve_storage_path(doc_to_edit.storage_path)
    ...
    @classmethod
    @performance_monitor("Repeat Groups Processing")
