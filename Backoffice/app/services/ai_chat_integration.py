...
def resolve_ai_trace_user_id(platform_context: Optional[Dict[str, Any]]) -> Optional[int]:
    """
    DB user id for AI reasoning traces and similar persistence.

    Mirrors the user_id resolution in _build_user_context: prefer Flask-Login's
    current_user, then platform_context (HTTP/SSE/WS set user_info.id from
    resolve_ai_identity(), including Bearer JWT when the session cookie is absent).
    """
    uid: Optional[int] = None
    if getattr(current_user, "is_authenticated", False):
        try:
            uid = int(getattr(current_user, "id", None))
    ...
    try:
        if platform_context and isinstance(platform_context, dict):
            ...
            raw = acc.get("user_id") or ui.get("id")
            if raw is not None:
                return int(raw)
    except (TypeError, ValueError) as e:
        logger.debug("resolve_ai_trace_user_id from platform_context failed: %s", e)
    return None
    ...
    def _build_user_context(
        ...
        if user_id is not None and access_level is None:
            ...
            try:
                ...
                logger.debug("derive access_level from platform_context failed: %s", e)
