...
_MAX_ANONYMOUS_HISTORY_ITEMS = 10
_MAX_ANONYMOUS_HISTORY_MSG_CHARS = 500


def _sanitize_anonymous_history(
    history: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Validate and cap conversation history from unauthenticated callers.

    Only keep the most recent items with known-safe keys to limit prompt
    injection surface and payload size from anonymous/proxy users.
    """
    if not history:
        return []
    sanitized: List[Dict[str, Any]] = []
    for item in history[-_MAX_ANONYMOUS_HISTORY_ITEMS:]:
        ...
        if not isinstance(msg, str):
            continue
        sanitized.append({
            "isUser": bool(item.get("isUser")),
            "message": msg[:_MAX_ANONYMOUS_HISTORY_MSG_CHARS],
        })
    return sanitized


def apply_anonymous_rules(parsed: ChatRequestParsed) -> ChatRequestParsed:
    """For anonymous users: clear conversation_id and persistence fields; cap client conversation_history."""
    return ChatRequestParsed(
        message=parsed.message,
        page_context=parsed.page_context,
        conversation_id=None,
        preferred_language=parsed.preferred_language,
        conversation_history=_sanitize_anonymous_history(parsed.conversation_history),
        client_message_id=None,
