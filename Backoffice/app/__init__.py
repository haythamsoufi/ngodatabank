...
def create_app(config_name=None):
    ...
    with app.app_context():
        try:
            ...
            from app.services.app_settings_service import ALLOWED_ENTITY_TYPE_GROUPS
    ...
    if app.debug:
        ...
        def test_static_file(filename):
            """Test route to verify static file accessibility. Only available in debug mode."""
            import os
            from flask import send_from_directory, abort
    ...
    def track_request_performance():
        ...
        from app.services.monitoring.system import track_request_performance
    ...
    # Authenticate via JWT Bearer token before Flask-Login session check.
    # This allows @login_required routes on /api/v1/ to accept mobile JWT tokens
    # without requiring per-route changes.  Runs before update_activity so that
    # JWT-authenticated users also get their session activity updated.
    @app.before_request
    ...
    # Register dedicated mobile API blueprint
            ...
            if not sys_manager_user:
                if test_country:
                    ...
                    if not os.environ.get('TEST_SYS_MANAGER_PASSWORD'):
                        app_instance.logger.info(
                            f"Created default system manager user '{test_sys_email}' with generated password. "
                            f"Set TEST_SYS_MANAGER_PASSWORD environment variable to use a fixed password."
                        )
                    else:
                        app_instance.logger.info(f"Created default system manager user '{test_sys_email}' and assigned Testland")
                else:
                    app_instance.logger.warning("Default country 'Testland' not found, cannot create default system manager.")
            else:
                app_instance.logger.info(f"Default system manager user '{test_sys_email}' already exists.")
