...
from app.utils.constants import CACHE_MAX_AGE_ONE_HOUR, SESSION_INACTIVITY_SECONDS
...
def create_app(config_name=None):
    ...
    # Now add our custom static route with cache headers
    # This is more efficient than using after_request on every request
    @app.route('/static/<path:filename>', endpoint='static')
    def send_static_file_with_cache(filename):
        """Serve static files with proper cache headers.

        Implements caching strategy:
        - Versioned files (?v=): 1 year cache, immutable
        - Unversioned JS/CSS: revalidate every time (max-age=0, must-revalidate)
          This is critical for ES module graphs: imported modules do not inherit the entry
          module's query string, so they would otherwise be "unversioned" and could stay stale.
        - Other unversioned static assets: 1 hour cache, must-revalidate

        Note: All templates should use static_url() to ensure versioning.
        Unversioned access is supported for backwards compatibility but gets shorter cache.

        Flask's send_static_file() automatically handles ETags for cache validation.
        """
        ...
        # This gives us full control over the response
        ...
        # Only add cache headers for successful responses and non-development environments
        if response.status_code == 200 and not is_development:
            # Clear any existing cache control headers (Flask might set defaults in dev mode)
            response.headers.pop('Cache-Control', None)
            response.headers.pop('Pragma', None)
            response.headers.pop('Expires', None)

            # Determine cache duration based on file type and versioning
            ...
            if any(path_lower.endswith(ext) for ext in static_extensions):
                # Check if versioned (has ?v= query param)
                query_string = req.query_string.decode('utf-8', errors='ignore')
                if 'v=' in query_string:
                    # Long cache for versioned assets (1 year)
                    # immutable flag tells browsers/CDNs the file will never change
                    response.cache_control.max_age = 31536000  # 1 year in seconds
                    response.cache_control.public = True
                    response.cache_control.immutable = True
                else:
                    # Unversioned assets: safest behavior depends on type.
                    #
                    # IMPORTANT: ES module imports do not automatically carry over the ?v= from the
                    # entry script. That means modules imported by a versioned entry module may still
                    # be fetched as "unversioned". To avoid stale-client issues after deploy, we
                    # force JS/CSS to revalidate every time; other assets keep a short cache.
                    if path_lower.endswith(('.js', '.css')):
                        response.cache_control.max_age = 0
                        response.cache_control.public = True
                        response.cache_control.must_revalidate = True
                    else:
                        # Shorter cache for other unversioned assets (1 hour)
        ...
        # Mark response to skip security headers cache override
        # This ensures our cache headers aren't overridden by security middleware
    ...
    try:
        ...
        elif selected_config_name == 'production':
            # Production: default to empty (no CORS) unless explicitly configured
            cors_origins = []
            app.logger.warning("CORS_ALLOWED_ORIGINS not set in production. CORS disabled for security.")
        else:
        ...
        app.logger.warning("CORS not enabled - Flask-CORS package not available. Install with: pip install Flask-Cors==4.0.0")
    ...
    verbose_debug = app.config.get('VERBOSE_FORM_DEBUG', False)
    ...
    if not upload_folder:
        # Fallback to default if UPLOAD_FOLDER is empty
    ...
    app.jinja_env.filters['fromjson'] = fromjson_filter # NEW: Register the fromjson filter
    app.jinja_env.filters['js'] = js_filter  # Safe JS/JSON literal output (alias for templates)

    # Register additional custom filters from utils
    ...
    # Expose user_has_permission to templates
    try:
    ...
    # Expose language config to templates
    ...
    # Emoji flags for any language code (always available; never silently missing)
    try:
        ...
        # Safe fallbacks
        app.jinja_env.globals['language_flag_emoji'] = lambda _code=None: "🏳️"
        app.jinja_env.globals['language_flag_twemoji_svg_url'] = lambda _code=None: None
    # Language name helpers exposed to every template:
    #   language_endonym  → locale-aware name (e.g. "French" when viewing in English).
    #                       This is the default used across all templates.
    #   language_native_name → native/endonym name (e.g. "Français" regardless of viewer).
    #                          Used only in the nav-bar language switcher.
    try:
    ...
    app.jinja_env.globals['ENABLED_ENTITY_TYPES'] = app.config.get('ENABLED_ENTITY_TYPES', getattr(config_class, 'ENABLED_ENTITY_TYPES', ['countries', 'ns_structure', 'secretariat']))

    # Organization branding (load from database settings)
    # Use functions that will be called on each request to get localized values
    ...
    # Store full branding dict
    org_branding = get_org_branding()
    app.jinja_env.globals['ORGANIZATION_BRANDING'] = org_branding

    # Add context processor to make localized organization names available in all templates
    @app.context_processor
    def inject_org_branding():
        """Inject localized organization branding into all templates."""
        try:
            ...
            # IMPORTANT: Always compute branding per-request.
            # `app.jinja_env.globals['ORGANIZATION_BRANDING']` is set at startup and can become stale
            # (e.g., if DB isn't ready during boot or in multi-process deployments).
    ...
    # RBAC helpers for templates (legacy-free)
    @app.context_processor
    def inject_rbac_helpers():
        ...
        Usage in Jinja:
        - {{ has_permission('admin.users.view') }}
        - {{ current_user_has_role('system_manager') }}
        - {{ user_has_role(user, 'system_manager') }}
        """
        try:
            from app.services.authorization_service import AuthorizationService
        except Exception as e:
        ...
        def get_admin_gate_permissions():
            """Return the list of admin gate permissions for template use."""
            try:
                if AuthorizationService is None:
                    return []
        ...
        def is_admin_user():
            """Check if current user is admin (system manager or has any admin gate permission)."""
            try:
                if AuthorizationService is None:
                    return False
        ...
        def user_access_level(user) -> str:
            """
            Legacy-free access classification for UI.
            Returns one of: system_manager | admin | focal_point | user | public
            """
            try:
    ...
    # Expose AI chat config to templates (widget visibility)
    app.jinja_env.globals['CHATBOT_ENABLED'] = app.config.get('CHATBOT_ENABLED', True)

    # Expose ASSET_VERSION to templates for JavaScript helper
    app.jinja_env.globals['ASSET_VERSION'] = app.config.get('ASSET_VERSION')

    # Expose config object to templates (for DEBUG and other config access)
    app.jinja_env.globals['config'] = app.config

    # Static file cache-busting helper
    from flask import url_for
    def static_url(filename):
        """Generate static file URL with cache-busting version parameter.

        Uses ASSET_VERSION (generated once per app start) so restarting the app
        refreshes cached JS/CSS (including Service Worker caches).
        """
    ...
    # Add slugify_age_group function for age group handling (canonical implementation)
    from app.utils.form_processing import slugify_age_group
    app.jinja_env.globals['slugify_age_group'] = slugify_age_group

    # Register EntityService as template global
    from app.services.entity_service import EntityService
    app.jinja_env.globals['EntityService'] = EntityService

    # Expose CSP nonce function to templates for CSP nonce migration
    ...
    # Register date formatting filters for localized month names and timezone conversion
    ...
    def format_datetime_localized_filter(dt, format='medium', format_time='medium'):
        """Format datetime with localized month names."""
        if not dt:
            return ''
        try:
            return format_datetime(dt, format=format, format_time=format_time)
        except Exception as e:
    ...
    def datetime_iso_filter(dt):
        """
        Convert datetime to ISO format string for JavaScript processing.
        Returns empty string if dt is None.

        Usage in templates:
            <span data-datetime="{{ some_datetime|datetime_iso }}">{{ some_datetime|format_datetime_localized }}</span>

        The JavaScript DateTimeUtils will convert to user's local timezone.
        """
        if not dt:
            return ''
        try:
    ...
    def datetime_local_filter(dt, format='datetime', css_class=''):
        """
        Generate a span element with data-datetime for client-side timezone conversion.

        This filter outputs an HTML span that will be automatically converted to the
        user's local timezone by DateTimeUtils.js.

        Args:
            dt: The datetime object (stored in UTC)
            format: Format type for JavaScript (datetime, date, time, datetimeShort, etc.)
            css_class: Additional CSS classes for the span

        Usage in templates:
            {{ some_datetime|datetime_local }}
            {{ some_datetime|datetime_local('date') }}
            {{ some_datetime|datetime_local('datetime', 'text-sm text-gray-600') }}

        Output:
            <span data-datetime="2024-01-15T10:30:00Z" data-datetime-format="datetime" class="...">
        ...
        try:
            ...
            # Format fallback display (will be replaced by JS)
            try:
                if format == 'date':
                    ...
                    fallback = format_datetime(dt, format='short', format_time='short')
            except Exception as e:
            ...
            # Return Markup so it's not escaped
            return Markup(
    ...
    def bad_request(error):
        ...
        return render_template('errors/error.html',
                             ...
                             style_nonce=get_style_nonce()), 400
    ...
    def unauthorized(error):
        ...
        return render_template('errors/error.html',
                             ...
                             error_details=str(error) if app.config.get('DEBUG') else None,
                             current_user=current_user,
                             style_nonce=get_style_nonce()), 401
    ...
    def forbidden(error):
        ...
        return render_template('errors/error.html',
                             ...
                             error_details=str(error) if app.config.get('DEBUG') else None,
                             current_user=current_user,
                             style_nonce=get_style_nonce()), 403
    ...
    def not_found(error):
        ...
        return render_template('errors/error.html',
                             ...
                             error_details=str(error) if app.config.get('DEBUG') else None,
                             current_user=current_user,
                             style_nonce=get_style_nonce()), 404
    ...
    def internal_error(error):
        ...
        # Notify system managers about critical errors (only in production or when enabled)
        # Skip notification in DEBUG mode to avoid spam during development
        if not app.config.get('DEBUG'):
            try:
                ...
                # Avoid touching ORM-backed attributes here: during error handling the SQLAlchemy
                # session may be rolled back/removed and `current_user` can be detached/expired.
                user_id = None
                ...
                # Create security event for critical error
                SecurityMonitor.log_security_event(
                    ...
                    context_data={
                        ...
                        'traceback': error_traceback[:1000]  # Limit traceback length
                    },
                    user_id=user_id
                )

                # Send email alert to system managers
                try:
                    ...
                    if managers:
                        ...
                        if manager_emails:
                            ...
                            success = send_security_alert(
                                ...
                                user_id=user_id,
                                timestamp=utcnow().isoformat(),
                                recipients=manager_emails  # Pass manager emails directly
                            )
                            if success:
        ...
        return render_template('errors/error.html',
                             ...
                             error_details=str(error) if app.config.get('DEBUG') else None,
                             current_user=current_user,
                             style_nonce=get_style_nonce()), 500
    ...
    def bad_gateway(error):
        ...
        return render_template('errors/error.html',
                             ...
                             error_details=str(error) if app.config.get('DEBUG') else None,
                             current_user=current_user,
                             style_nonce=get_style_nonce()), 502
    ...
    def service_unavailable(error):
        ...
        return render_template('errors/error.html',
                             ...
                             error_details=str(error) if app.config.get('DEBUG') else None,
                             current_user=current_user,
                             style_nonce=get_style_nonce()), 503
    ...
    # Initialize notification cleanup scheduler (only in production, not during migration)
    # Defer to background thread to avoid blocking startup (especially on Windows)
    # Only initialize in the reloader process (child) or production (not in parent watcher)
    # In development with reloader: parent process watches files, child process runs server
    # We only want scheduler in the child process (WERKZEUG_RUN_MAIN == 'true')
    should_init_scheduler = (
        not app.config.get('TESTING', False)
        and not os.environ.get('RUNNING_MIGRATION')
        and (not app.debug or is_reloader)  # Only in reloader process if debug mode
    )

    if should_init_scheduler:
        # Check if scheduler already exists (prevent duplicate initialization)
        if not hasattr(app, 'scheduler') or app.scheduler is None:
            ...
            def scheduler_init_task():
                try:
                    import time
                    time.sleep(0.1)  # Small delay to let app finish initializing
                    with app.app_context():
                        # Use lock to prevent concurrent initialization
                        with app._scheduler_lock:
                            # Double-check scheduler doesn't exist (race condition protection)
                            ...
                            # Store scheduler reference BEFORE starting to prevent race condition
                            # Use a placeholder to mark initialization in progress
                            app.scheduler = scheduler

                            # Wrap jobs to ensure they run within the Flask application context
                            ...
                            # Daily notification cleanup at 2 AM
                            scheduler.add_job(
                            ...
                            # Hourly inactive session cleanup
                            scheduler.add_job(
                            ...
                            def _retry_failed_emails_with_context():
                                with app.app_context():
                                    try:
                                        from app.utils.transactions import atomic
                                        from app.utils.email_delivery_tracking import get_pending_retries
                                        from app.utils.notification_emails import retry_email_delivery_log
                            ...
                            def _send_digest_emails_with_context():
                                with app.app_context():
                                    try:
                                        from app.utils.transactions import atomic
                                        from app.utils.notification_emails import send_notification_emails
                                        with atomic(remove_session=True):
                                            send_notification_emails()
                                    except Exception as e:
                                        app.logger.error(f"Scheduled digest email job failed: {e}", exc_info=True)

                            # Run digest email check every minute
                            # The function itself checks if it's time for each user based on their preferences
                            scheduler.add_job(
                                func=_send_digest_emails_with_context,
                                trigger="interval",
                                minutes=1,
                                id='check_and_send_digest_emails',
                                replace_existing=True
                            )
                            ...
                            def _process_scheduled_notifications_with_context():
                                with app.app_context():
                                    try:
                                        from app.utils.transactions import atomic
                                        from app.utils.notification_scheduling import process_scheduled_notifications
                                        with atomic(remove_session=True):
                            ...
                            if not scheduler.running:
                                ...
                                app.logger.debug(f"Notification cleanup scheduler started (runs daily at 2 AM) [PID: {process_id}]")
                            else:
                                app.logger.debug("Scheduler was already running")

                except Exception as e:
                    app.logger.warning(f"Could not start notification scheduler: {e}")
                    # Clear scheduler reference if initialization failed
                    if hasattr(app, 'scheduler'):
                        app.scheduler = None

            # Start scheduler initialization in background thread
