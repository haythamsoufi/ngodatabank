...
(function () {
    ...
    function init() {
        ...
        window.addEventListener('chatbot-optimistic-title', function (ev) {
            var d = ev && ev.detail;
            if (!d || !d.title) return;
            var titleText = String(d.title).trim();
            if (!titleText) return;
            var titleEl = document.getElementById(TITLE_ID);
            if (titleEl) titleEl.textContent = titleText;
            var bot = getChatbot();
            var activeId = bot && bot.getActiveConversationId && bot.getActiveConversationId();
            var cid = d.conversationId ? String(d.conversationId) : '';
            if (cid && activeId && String(activeId) === cid) {
                for (var i = 0; i < lastConversations.length; i++) {
                    var c = lastConversations[i];
                    if (c && String(c.id) === cid) {
                        lastConversations[i] = Object.assign({}, c, { title: titleText });
                        break;
                    }
                }
                applySearchFilter();
