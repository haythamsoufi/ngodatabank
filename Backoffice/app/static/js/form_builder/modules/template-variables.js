...
async function _fetchJson(url) {
    ...
    if (apiFn) return apiFn(url);
    const r = await ((window.getFetch && window.getFetch()) || fetch)(url);
...
export class TemplateVariablesManager {
    ...
    updateVariablesButton() {
        ...
        if (count === 0) {
            // Grey out button when no variables
            btn.classList.remove('bg-purple-600', 'hover:bg-purple-700', 'text-white');
            btn.classList.add('bg-gray-100', 'hover:bg-gray-100', 'text-gray-500', 'border', 'border-gray-300');
            btn.disabled = false; // Keep enabled so modal can still be opened
        } else {
            // Normal purple styling when variables exist
            btn.classList.remove('bg-gray-100', 'hover:bg-gray-100', 'text-gray-500', 'border', 'border-gray-300');
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="source_template_id" class="w-full px-3 py-2 border border-gray-300 rounded-md" id="source-template-select">
                                    ...
                                    ${this.allTemplates.map(t => `<option value="${t.id}" ${existingVar?.source_template_id == t.id ? 'selected' : ''}>${t.name}</option>`).join('')}
                                </select>
                            </div>
