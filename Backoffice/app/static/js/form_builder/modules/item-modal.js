...
export const ItemModal = {
    ...
    setSanitizedHtml: function(container, html) {
        ...
        root.querySelectorAll('script, iframe, object, embed, style, meta, link').forEach((el) => el.remove());
    ...
    setupAjaxBeforeSubmitHook: function() {
        ...
        document.addEventListener('formBuilder:beforeAjaxSubmit', (evt) => {
            ...
            try {
                ...
                try { console.warn('[ItemModal] beforeAjaxSubmit prepare failed', e); } catch (_e) {}
            }
        });
    },
    ...
    switchItemType: function(itemType, optionalQuestionType) {
        ...
        try { console.debug('[ItemModal:Privacy] switchItemType ->', itemType); } catch (e) {}
        ...
        try { console.debug('[ItemModal:Privacy] calling ensurePrivacyField()'); } catch (e) {}
    ...
    ensurePrivacyField: function() {
        ...
        try { console.debug('[ItemModal:Privacy] ensurePrivacyField: propertiesSection exists?', !!propertiesSection); } catch (e) {}
        if (!propertiesSection) return;
        const propertiesContent = propertiesSection.querySelector('.grid.grid-cols-2.gap-6.items-center');
        try { console.debug('[ItemModal:Privacy] ensurePrivacyField: propertiesContent exists?', !!propertiesContent); } catch (e) {}
        if (!propertiesContent) return;
        // Avoid duplicates
        if (propertiesContent.querySelector('#item-privacy-select')) {
            try { console.debug('[ItemModal:Privacy] ensurePrivacyField: privacy select already present'); } catch (e) {}
            return;
        }
        ...
        try { console.debug('[ItemModal:Privacy] ensurePrivacyField: appended select with default value:', select.value); } catch (e) {}
    },
    ...
    ensureAllowOver100Field: function(itemType) {
        console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: called with itemType =', itemType);
        if (!this.modalElement) {
            console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: modalElement not found');
            return;
        }
        const propertiesSection = this.modalElement.querySelector('.mb-3.border-t.border-gray-200.pt-4');
        if (!propertiesSection) {
            console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: propertiesSection not found');
            return;
        }
        const propertiesContent = propertiesSection.querySelector('.grid.grid-cols-2.gap-6.items-center');
        if (!propertiesContent) {
            console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: propertiesContent not found');
            return;
        }

        // Check if this is a percentage item
        const isPercentage = this.isPercentageItem(itemType);
        console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: isPercentage =', isPercentage);
        ...
        if (existingCheckbox) {
            ...
            console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: removing existing checkbox, preserving checked state =', preservedCheckedState);
            ...
            console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: using pending value =', preservedCheckedState);
            this._pendingAllowOver100Value = undefined; // Clear after use
        }

        // Only add checkbox for percentage items
        if (!isPercentage) {
            console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: not a percentage item, skipping checkbox creation');
            return;
        }

        console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: creating checkbox');
        ...
        if (preservedCheckedState !== undefined) {
            ...
            console.debug('[ItemModal:AllowOver100] ensureAllowOver100Field: restored checked state =', preservedCheckedState);
        }
    ...
    isPercentageItem: function(itemType) {
        console.debug('[ItemModal:AllowOver100] isPercentageItem: checking itemType =', itemType);
        if (itemType === 'question') {
            const questionTypeSelect = this.modalElement.querySelector('#item-question-type-select');
            const unitInput = this.modalElement.querySelector('#item-question-unit');
            console.debug('[ItemModal:AllowOver100] isPercentageItem: questionTypeSelect value =', questionTypeSelect ? questionTypeSelect.value : 'not found');
            console.debug('[ItemModal:AllowOver100] isPercentageItem: unitInput value =', unitInput ? unitInput.value : 'not found');
            if (questionTypeSelect && questionTypeSelect.value === 'percentage') {
                console.debug('[ItemModal:AllowOver100] isPercentageItem: detected percentage from question type');
                return true;
            }
            if (unitInput && unitInput.value && unitInput.value.toLowerCase().includes('percentage')) {
                console.debug('[ItemModal:AllowOver100] isPercentageItem: detected percentage from unit');
                return true;
            }
            ...
            console.debug('[ItemModal:AllowOver100] isPercentageItem: indicatorTypeSelect value =', indicatorTypeSelect ? indicatorTypeSelect.value : 'not found');
            console.debug('[ItemModal:AllowOver100] isPercentageItem: indicatorUnitSelect value =', indicatorUnitSelect ? indicatorUnitSelect.value : 'not found');
            console.debug('[ItemModal:AllowOver100] isPercentageItem: bankSelect value =', bankSelect ? bankSelect.value : 'not found');
            ...
            if (indicatorTypeSelect && indicatorTypeSelect.value &&
                ...
                console.debug('[ItemModal:AllowOver100] isPercentageItem: detected percentage from indicator type');
                return true;
            }
            if (indicatorUnitSelect && indicatorUnitSelect.value &&
                indicatorUnitSelect.value.toLowerCase().includes('percentage')) {
                console.debug('[ItemModal:AllowOver100] isPercentageItem: detected percentage from indicator unit');
                return true;
            }
            ...
            if (bankSelect && bankSelect.value) {
                ...
                if (indicatorId) {
                    ...
                    if (indicator) {
                        ...
                        console.debug('[ItemModal:AllowOver100] isPercentageItem: indicator from bank - type =', indicatorType, 'unit =', indicatorUnit);
                        if (indicatorType.includes('percentage') || indicatorUnit.includes('percentage')) {
                            console.debug('[ItemModal:AllowOver100] isPercentageItem: detected percentage from indicator bank');
                            return true;
                        }
                    }
                }
        ...
        console.debug('[ItemModal:AllowOver100] isPercentageItem: not a percentage item');
        return false;
    },
        ...
        this.loadBasePluginTemplate().then(() => {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (data.script) {
                        console.warn('[ItemModal] Plugin returned script text, which is deprecated and will be removed. Use static JS files instead.');
    ...
    populateIndicatorForm: function(itemData) {
        console.debug('[ItemModal:AllowOver100] populateIndicatorForm: called with itemData =', itemData);
        ...
        console.debug('[ItemModal:AllowOver100] populateIndicatorForm: calling ensureAllowOver100Field after populating indicator');
        ...
        if (this.currentItemType === 'indicator') {
            ...
            if (disaggContainer) {
                ...
                try {
                    ...
                    console.debug('[ItemModal] serialize disaggregation', {
    ...
    populateQuestionForm: function(itemData) {
        console.debug('[ItemModal:AllowOver100] populateQuestionForm: called with itemData =', itemData);
        ...
        console.debug('[ItemModal:AllowOver100] populateQuestionForm: calling ensureAllowOver100Field after populating question');
    ...
    populateCommonFields: function(itemData) {
        ...
        try {
            console.debug('[ItemModal:Privacy] populateCommonFields: privacySelect exists?', !!privacySelect);
        } catch (e) {}
        ...
        if (privacySelect) {
            ...
            try {
                console.debug('[ItemModal:Privacy] populateCommonFields: incoming item privacy (top-level):', itemData && itemData.privacy);
                console.debug('[ItemModal:Privacy] populateCommonFields: incoming item privacy (config):', itemData && itemData.config && itemData.config.privacy);
            } catch (e) {}
            try {
                if (itemData) {
                    if (itemData.privacy) {
            ...
            try { console.debug('[ItemModal:Privacy] populateCommonFields: resolved privacy before normalize:', privacyValue); } catch (e) {}
            ...
            try { console.debug('[ItemModal:Privacy] populateCommonFields: final privacy set:', privacySelect.value); } catch (e) {}
        }

        // Allow Over 100% checkbox (from config)
        console.debug('[ItemModal:AllowOver100] populateCommonFields: itemData.config =', itemData && itemData.config);
        console.debug('[ItemModal:AllowOver100] populateCommonFields: currentItemType =', this.currentItemType);
        ...
        if (itemData && itemData.config && itemData.config.allow_over_100 !== undefined) {
            console.debug('[ItemModal:AllowOver100] populateCommonFields: allow_over_100 value =', itemData.config.allow_over_100, 'type =', typeof itemData.config.allow_over_100);
            ...
            console.debug('[ItemModal:AllowOver100] populateCommonFields: resolved allowOver100 =', allowOver100);
        } else {
            console.debug('[ItemModal:AllowOver100] populateCommonFields: allow_over_100 not found in config or config is missing');
        }

        // Check if checkbox already exists
        let allowOver100Checkbox = this.modalElement.querySelector('#item-allow-over-100');
        console.debug('[ItemModal:AllowOver100] populateCommonFields: checkbox found?', !!allowOver100Checkbox);

        // Only call ensureAllowOver100Field if checkbox doesn't exist yet
        if (!allowOver100Checkbox) {
            console.debug('[ItemModal:AllowOver100] populateCommonFields: checkbox not found, calling ensureAllowOver100Field');
        ...
        if (allowOver100Checkbox) {
            ...
            console.debug('[ItemModal:AllowOver100] populateCommonFields: set checkbox.checked =', allowOver100Checkbox.checked);
        } else {
            console.debug('[ItemModal:AllowOver100] populateCommonFields: checkbox still not found after ensureAllowOver100Field');
        }
    ...
    setupFormSubmission: function() {
        ...
        document.addEventListener('submit', (e) => {
            ...
            try {
                ...
                try { console.warn('[ItemModal] submit prepare failed', err); } catch (_e) {}
            }
        });
    },
