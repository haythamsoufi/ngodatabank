...
(function() {
    ...
    CustomSetFilter.prototype.init = function(params) {
        ...
        // If column has a valueGetter, we need to use it directly
        // AG Grid's filter valueGetter should handle this, but let's ensure it works
        if (this.colDef && this.colDef.valueGetter && typeof this.colDef.valueGetter === 'function') {
            ...
            this.valueGetter = function(node) {
                // First try AG Grid's valueGetter (which should handle column valueGetter)
                let value = originalValueGetter ? originalValueGetter(node) : undefined;
                // If that doesn't work, call column valueGetter directly
                if (value === undefined || value === null) {
                    try {
                        value = columnValueGetter({ node: node, data: node.data });
                    } catch (e) {
                        // Column valueGetter failed
                    }
                }
                return value;
            }.bind(this);
    ...
    CustomSetFilter.prototype.doesFilterPass = function(params) {
        // valueGetter expects params object, not just node
