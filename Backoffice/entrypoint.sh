...
PY
  ...
  SEED_TEST_DATA="$(echo "${SEED_TEST_DATA:-}" | tr '[:upper:]' '[:lower:]')"
  FLASK_CFG_LOWER="$(echo "${FLASK_CONFIG:-}" | tr '[:upper:]' '[:lower:]')"
  if [ "$SEED_TEST_DATA" = "true" ] || [ "$SEED_TEST_DATA" = "1" ]; then
    if [ "$FLASK_CFG_LOWER" = "production" ] || [ "$FLASK_CFG_LOWER" = "staging" ]; then
      echo "SEED_TEST_DATA ignored in $FLASK_CONFIG environment"
    else
      echo "=========================================="
